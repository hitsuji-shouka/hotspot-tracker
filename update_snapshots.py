...
OUT_DIRS = [ROOT / "public" / "data", ROOT / "dist" / "data"]
...
def main() -> dict:
    ...
    return {"updatedAt": now, "skillsCount": len(skills), "financeCount": len(finance)}
...
def run(ctx=None):
    ...
    summary = (
        ...
        f"财经快讯 {stats['financeCount']} 条（{stats['updatedAt']}）"
    )
