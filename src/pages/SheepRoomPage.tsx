...
export default function SheepRoomPage() {
  ...
  useEffect(() => {
    ...
    fetch('/api/room/status').then(response => response.json()).then(data => setAvailable(data.available === true)).catch(() => setAvailable(false))
  ...
  async function shop() {
    ...
    try {
      ...
      if (bagRef.current.length) setModal('haul')
    }
  }
  ...
  async function render() {
    if (!runId || !complete || rendering || renderAttempted || !products.length) return
  ...
  return <div className="lab-page sheep-room" data-category="all">
    ...
    <main className="room-stage" data-view={view} data-phase={phase}>
      ...
      <div className="room-controls">
        <div className="room-views" aria-label="观看视角"><button disabled={phase === 'zoom'} aria-pressed={view === 'room'} onClick={() => setView('room')}>全景</button><button disabled={phase === 'zoom'} aria-pressed={view === 'desk'} onClick={() => setView('desk')}>近景</button></div>
    ...
    <dialog ref={dialogRef} className={`room-dialog room-dialog-${modal}`} onCancel={() => setModal(null)} aria-labelledby="room-dialog-title">
      ...
      {modal === 'setup' && <form onSubmit={start}>
        ...
        <p className="room-dialog-intro">选好房间，再看它去宜家慢慢挑。照片、价格和心头好，都会带回来。</p>
      ...
      {modal === 'haul' && <>
        ...
        {product ? <><div className="room-haul-heading"><span>{String(activeItem + 1).padStart(2, '0')} / {products.length}</span><button className="lab-action" onClick={() => setModal('receipt')}>完整清单</button></div>
          <div className="room-haul-view"><button className="room-gallery-arrow" aria-label="上一件商品" disabled={activeItem <= 0} onClick={() => setActiveItem(index => index - 1)}><ChevronLeft /></button><Photo key={product.id} product={product} /><button className="room-gallery-arrow" aria-label="下一件商品" disabled={activeItem >= products.length - 1} onClick={() => setActiveItem(index => index + 1)}><ChevronRight /></button></div>
      ...
      {modal === 'receipt' && <>
        ...
        <div className="room-receipt-actions"><button className="lab-action" onClick={copyList}>{copied ? '清单已复制' : '复制清单'}</button><button className="lab-action" onClick={saveList}><Download size={16} />保存清单</button><button className="lab-action" onClick={() => setModal('haul')}>看商品图片</button></div>
        <div className="room-render-action"><p>把这一袋心头好，布置在一起。</p><small>使用完整清单和商品参考图，最后生成一次效果图。</small><button className="lab-action" disabled={!complete || rendering || renderAttempted || !products.length} onClick={render}>{rendering ? <><RefreshCw className="lab-spin" size={16} />正在布置…</> : renderAttempted ? image ? '效果图已生成' : '生成状态待确认' : '生成小屋效果图'}</button>{image && <button className="lab-action" onClick={() => setModal('image')}>查看效果图</button>}</div>
      </>}
