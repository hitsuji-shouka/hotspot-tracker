...
export default function AdminControl({ dark = false }: { dark?: boolean }) {
  ...
  async function submit(event: React.FormEvent) {
    ...
    catch (error) { setError(error instanceof Error ? error.message : '无法连接服务，请重试') }
  ...
  return <>
    <button className="inline-flex items-center gap-1.5 text-xs opacity-70 transition-opacity hover:opacity-100" disabled={busy} onClick={async () => {
      ...
      setBusy(true)
      try { await lockAdmin() } catch { adminMessage('退出失败，请检查网络后重试。') } finally { setBusy(false) }
    }}>{canEdit ? <LogOut className="h-3.5 w-3.5" /> : <LockKeyhole className="h-3.5 w-3.5" />}{canEdit ? '退出编辑' : '管理'}</button>
    <Dialog open={open} onOpenChange={close}>
      <DialogContent className={dark ? 'border-[#30363d] bg-[#161b22] text-[#c9d1d9]' : 'border-[#e5e0d8] bg-[#faf9f6] text-[#26221c]'}>
        <DialogHeader><DialogTitle>解锁编辑</DialogTitle><DialogDescription className={dark ? 'text-[#8b949e]' : 'text-[#777268]'}>有效期 30 天</DialogDescription></DialogHeader>
        <form onSubmit={submit} className="space-y-4">
          <div className="space-y-2"><label htmlFor="admin-password" className="text-sm">管理密码</label><input id="admin-password" type="password" autoComplete="current-password" required maxLength={1024} value={password} onChange={event => setPassword(event.target.value)} className="w-full rounded-md border border-current/20 bg-transparent px-3 py-2 text-base outline-offset-2" /></div>
          {ready && !configured && <p className="text-sm" role="status">暂未开放编辑</p>}
          {error && <p role="alert" className="text-sm text-red-500">{error}</p>}
          <div className="flex justify-end gap-2"><button type="button" className="rounded-md px-4 py-2 text-sm" onClick={() => close(false)}>取消</button><button type="submit" disabled={busy || !configured || !password} className={'rounded-md px-4 py-2 text-sm text-white disabled:opacity-50 ' + (dark ? 'bg-[#238636]' : 'bg-[#c2410c]')}>{busy ? '验证中…' : '解锁'}</button></div>
        </form>
      </DialogContent>
    </Dialog>
  </>
