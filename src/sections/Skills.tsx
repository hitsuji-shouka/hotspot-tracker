    ...
    <div className="space-y-5">
      ...
      <div className="flex items-center gap-2">
        <Button
          ...
          className={source === 'skillssh' ? 'bg-[#a371f7] hover:bg-[#a371f7]/85 text-white' : 'border-[#30363d] text-[#c9d1d9]'}
        >
          🏆 skills.sh 热榜
        </Button>
        <Button
          ...
          className={source === 'github' ? 'bg-[#a371f7] hover:bg-[#a371f7]/85 text-white' : 'border-[#30363d] text-[#c9d1d9]'}
        >
          🌱 GitHub 新星
        </Button>
      ...
      {loading ? (
        ...
        <div className="space-y-2">
          {filtered.map((s) => (
            ...
            >
              <span className="w-8 text-center font-mono text-sm text-[#8b949e] shrink-0">
                {s.rank ?? '—'}
              </span>
              ...
              <a
                ...
                rel="noreferrer"
                title="查看 GitHub 仓库"
                className="p-1.5 rounded-md text-[#8b949e] hover:text-white hover:bg-[#30363d] shrink-0"
              >
...
function GithubSkills({ keyword, isFavorite, onToggleFavorite }: Props) {
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center gap-3 flex-wrap">
        <Tabs value={range} onValueChange={(v) => setRange(v as SkillRange)}>
        ...
        <Button
          ...
          onClick={() => load(true)}
          disabled={loading}
          className="ml-auto border-[#30363d] text-[#c9d1d9]"
        >
          <RefreshCw className={`w-4 h-4 mr-1 ${loading ? 'animate-spin' : ''}`} />
          刷新
        </Button>
